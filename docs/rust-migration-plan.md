...
pub(crate) struct RuntimeKernel {
    ...
    pub pause_started_ns: Option<u64>,
    pub focus_pause_started_ns: Option<u64>,
...
drain_due(&mut kernel, elapsed) {
    ...
    while let Some(next) = kernel.pending_releases.peek() {
        ...
        let res = send::emit(&input_cache, &[next.scan_code], true);
    ...
    while kernel.cursor < kernel.intents.len() {
        ...
        let res = send::emit(&input_cache, &playable.scan_codes, false);
        kernel.counters.accumulate(&res);
...
pub(crate) struct EmitResult {
    ...
    pub start_perf_ns: u64,
    pub completed_perf_ns: u64,      // right after SendInput returned
}
...
pub(crate) fn emit(
    ...
    loop {
        ...
        if sent == remaining.len() as u32 { break; }
        if sent > 0 {
            // partial send — chord split, retry remaining
            remaining = &remaining[sent as usize..];
            zero_retries = 0;
        } else {
            zero_retries += 1;
            if zero_retries >= 3 {
                return Err(OsError::ZeroProgressSendInput);
            }
...
- **Bounded channel:** prevents Rust writer from unbounded heap growth if Python telemetry consumer is slow. Capacity 1024. Drop policy: drop **the oldest pending batch** (LIFO drop) on overflow — late tail data is more valuable than stale upsampled mid-playback batches.
...
2. Implement `send/emit.rs` (f64 loop: SendInput → partial-retry → zero-progress raise).
...
4. Implement `focus/guard.rs` (foreground check with 2 ms cache — the same TTL as Python `DispatchHealthMonitor._focus_cache_ttl_us`).
