...
try {
    ...
    foreach ($scenarioName in $defaultScenarios) {
        $cases += [pscustomobject]@{ scenario = $scenarioName; margin = 800; tolerance = 500 }
    }
    foreach ($toleranceValue in @(500,1000,2000,5000)) {
        $cases += [pscustomobject]@{ scenario = 'timing-margin-sweep'; margin = 800; tolerance = $toleranceValue }
    }
