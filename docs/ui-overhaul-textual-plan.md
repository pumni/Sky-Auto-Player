...
**Cổng kiểm duyệt P2:**
- [ ] Mọi chức năng `commands` của picker cũ có mặt và tương đương.
- [ ] Đổi profile/tempo/fps/theme **persist** đúng (kiểm `config.json` thay đổi).
- [ ] Pilot test cho mỗi modal + palette (mở, chọn, áp dụng, đóng).
- [ ] Theme map đủ 5 biến thể, không vỡ layout.
...
**Cổng P3:** [ ] calibration/help/reload tương đương cũ; [ ] warm cache không chặn first paint; [ ] test.
...
**Cổng P4:** [ ] query rỗng vẫn A→Z; [ ] gõ sai 1-2 ký tự vẫn ra kết quả hợp lý; [ ] benchmark lọc <16ms/keystroke với 107+ bài; [ ] test ranking.
...
**Cổng P5:**
- [ ] `auto` chọn textual đúng môi trường, fallback classic khi không TTY/terminal yếu.
- [ ] Smoke đóng gói lại exe, chạy thực tế, chọn + phát OK.
- [ ] README/help cập nhật. Không gỡ prompt_toolkit trong cùng PR cutover.
