...
* **Completion Anchor:** To protect key-down visibility against OS injection latency, the coordinator calculates release limits dynamically:
  ...
  ```
* **Conflict Resolution:** If a same-key down event is scheduled while the previous generation of that key is still active (e.g. due to compaction or dispatch delay), the coordinator applies the configured chord-conflict policy. `degraded` drops only conflicting keys, `drop_chord` drops the complete authored chord, and `strict`/`abort` records a controlled terminal error and aborts playback. Accuracy-first callers should use `drop_chord` or `strict`.
