...
* **Release recovery timeline:** While a failed release is retrying, authored dispatch is held behind the pending release. After successful recovery, the remaining authored schedule is shifted by the recovery interval so same-key work is sent after release without an overdue catch-up burst.
