...
* **Current rollout:** Rust is the default for the eligible real Windows sender path. `SKY_USE_PYTHON_DISPATCH=1` is the explicit diagnostic rollback switch; the legacy `SKY_USE_RUST_DISPATCH=1` remains accepted but is no longer required. Missing or incompatible native metadata falls back to Python with a telemetry warning, while `SKY_REQUIRE_RUST_DISPATCH=1` makes that condition fail closed for CI/validation.
...
* **Packaging gate:** release CI builds and imports the version-specific wheel before Python tests. The wheel gate requires a clean checkout and, when present, an exact `GITHUB_SHA`/`git HEAD` match; explicitly marked local dirty builds carry a `-dirty` build-commit suffix. `build_app.py` embeds that exact build ID in the frozen Python package, and the runtime accepts a native extension only when its schema, ABI, no-GIL/Win32 capabilities, and embedded build ID match. Bare source checkouts may omit the generated expectation for local development. The frozen executable runs `--selftest-rust`, which imports the bundled module and completes a mock worker without emitting real input.
...
* **Partial note-on no *late* retry (G5):** If `SendInput` returns `sent < n` for a musical note-on, the remainder is retried **once, immediately (no sleep)**, keeping it in the same sender-side dispatch window. Whatever still does not land is dropped (`DROPPED_BACKEND`, tagged `partial_note_on`).
* **Failed note-off ownership:** A pending release is not discarded when `SendInput` makes no progress. The coordinator requeues the still-active generation with bounded `2/5/10/20 ms` backoff for up to eight attempts; exhaustion enters error recovery and performs full-instrument release before cancellation. Same-key downs remain blocked while that release is pending.
