...
the private updater key and Authenticode credentials remain external,
fail-closed release inputs.

Qualification binds the exact NSIS installer and Tauri signature bytes by
SHA-256 and retains the accepted Authenticode, SPDX SBOM, provenance, clean
worktree, install/launch/uninstall, and previous-v4-to-candidate-v4 fixture
evidence. Release orchestration remains subject to the dedicated v4 release
authority and its reviewed promotion policy.
