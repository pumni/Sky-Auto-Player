...
* [2026-07_core-dispatch-refactor-and-isolation-plan.md](2026-07_core-dispatch-refactor-and-isolation-plan.md) — **In progress (Phases 0–1 landed).** Phased refactor from the 2026-07-16 core review: fixes confirmed defects (dead pre-down focus gate, pause double-count, RT-thread telemetry flush, per-send OpenProcess focus refresh), reduces dispatch-thread CPU floors, and makes the core boundary structural (`orchestration/core/` package + typed ports) as the seam for the Rust port. **Must be completed before rust-migration-plan.md Phase 0 starts** (Phase 5 of this plan patches that document's §8 emit policy). Phase 0 harness: `tests/test_golden_dispatch_timeline.py`, `tests/test_telemetry_summary_schema.py`, [perf-baselines/2026-07-refactor-baseline.md](perf-baselines/2026-07-refactor-baseline.md). Phase 1: A1 focus gate, A2 pause state machine, A6a clock injection, A6b estimator no-op guard.
* [rust-migration-plan.md](rust-migration-plan.md) — Proposed plan: migrate the real-time dispatch hot path (send/wait/runtime) from Python ctypes into a dedicated Rust dispatch worker via PyO3, keeping Python for orchestration/UI only. **Blocked on** [2026-07_core-dispatch-refactor-and-isolation-plan.md](2026-07_core-dispatch-refactor-and-isolation-plan.md) (its §8 emit pseudo-code currently contradicts the shipped musical no-retry policy; see that plan's Phase 5).
