...
* [distribution-and-update.md](distribution-and-update.md) — Distribution model, update architecture, and release contracts for Sky Auto Player (tracks the `pyproject.toml` `[project].version`, currently 2.4.2).

---

## 2. Active References & Experiments
* [plan/2026-07-24_rename-update-cutover-v2.4.2-plan.md](plan/2026-07-24_rename-update-cutover-v2.4.2-plan.md) — **Implemented (shipped in 2.4.2).** Post-rename update cutover for **v2.4.2**: closed the live asset-name gap (`Sky-Player-v*` still on GitHub vs code expecting `Sky-Auto-Player-v*`), dual-publish a **legacy bridge zip** so pre-rename `Sky-Player.exe` installs can run existing `updater.bat` once, dual-name identity in `updater.bat`/`updater.ps1`, restore normative `docs/distribution-and-update.md`, and publish the five-asset release contract. AI-safe phased execution (tests → updater → build_app bridge → release.yml → docs → version → E2E → tag). Does **not** reintroduce in-app auto-apply. P0/security gates unchanged.
* [plan/2026-07-23_dispatch-chord-timing-residual-improvement-plan.md](plan/2026-07-23_dispatch-chord-timing-residual-improvement-plan.md) — **Implemented and verified (2026-07-24).** Consume [dispatch-chord-timing-residual-review-2026-07-23.md](dispatch-chord-timing-residual-review-2026-07-23.md): phased fixes for the residual chord-lifecycle / shutdown / cache / wait / warmup defects left after the core dispatch refactor. **R1 chord stagger lifecycle** fixed in `scheduler.py` Stage 1.5 (per-key offset applied before raw DOWN/UP construction). **R2 join-timeout** fail-safe (`PLAYBACK_SHUTDOWN_TIMEOUT` token; handles/cache left alone under a live dispatcher). **R3 lead-cache** element-level type rejection + widened `ema_residual` domain (atomic validate-then-apply). **R4 `WAIT_FAILED`** bounded 2 ms poll instead of full-gap spin. **R5 warmup** moved onto the real pre-send path inside `_wait_until_runtime_deadline()`. **R6 (stale `now_us`)** deferred — no production caller enables `late_pulse_drop_threshold_us`. **R7/R8** doc-wording + `timing-profile-frame-model.md` reconciled to actual code (`audience_safe = 1.5`, `min_hold_margin_us = 500 µs`; baseline table recomputed). Verification pass finished a half-written Phase 0 runtime test, cleared 21 Ruff violations, and reconciled the Phase 6 doc drift. P0 untouched in behaviour (only docstrings in `inputs.py`); security audit green.
