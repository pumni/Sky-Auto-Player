...
* [timing-principles.md](timing-principles.md) — Timing design, same-key feasibility limits, the completion-anchor contract, and adaptive-lead/floor interaction.
...
* [distribution-and-update.md](distribution-and-update.md) — Canonical portable distribution, native Rust out-of-process updater, preserve-list, transaction/rollback contract, and release assets (tracks the `pyproject.toml` `[project].version`, currently 3.1.0).
...
* [plan/sky-auto-player-clean-self-update-plan/00_README_VI.md](plan/sky-auto-player-clean-self-update-plan/00_README_VI.md) — **Implemented in this worktree; release qualification remains fail-closed.** Native Rust updater, RegisterHotKey handoff, clean script-updater removal, signed-byte manifest order, and conservative transaction/recovery rules are now the active update contract. Authenticode/Defender/packaged E2E evidence must be captured on the release runner before publication.
