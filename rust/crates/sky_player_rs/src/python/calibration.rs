...
//! tool surface that blocks the calling thread while collecting sender-side
//! completion samples. Python callers should invoke it from a worker thread or
//! subprocess to avoid blocking the UI.
//!
//! # Evidence scope
//!
//! All data produced here is **Rust/SendInput sender completion hold shrink**
//! evidence. It is **not** Sky/game-observed latency. The `evidence_kind`
//! field in the JSON output says so explicitly.
