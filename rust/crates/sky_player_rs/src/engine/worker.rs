...
pub(crate) use cleanup::{
    cancel_coordinator_or_terminal, clean_completion_proven, describe_release_outcome,
    record_termination_error, release_runtime_outcome, release_state_verified, suspend_live_input,
};
#[cfg(test)]
...
impl<'a> Worker<'a> {
    ...
    pub(super) fn run(self) -> u8 {
        ...
        *priority_acquired.lock() = priority_guard.acquired().to_string();
        let waiter =
            HybridWaiter::with_options(config.enable_waitable_timer, config.enable_event_wait);
        *metrics.wait_strategy_acquired.lock() = waiter.mode().to_string();
        ...
        let focus_restore_grace_ticks =
            match qpc_clock.duration_from_us(config.focus_restore_grace_us) {
                Ok(ticks) => ticks,
                Err(error) => {
                    return admission_failure(
                        &mut backend,
        ...
        let lease_timeout_ticks =
            match qpc_clock.duration_from_us(config.supervisor_lease_timeout_us) {
                Ok(ticks) => ticks,
                Err(error) => {
                    return admission_failure(
                        &mut backend,
        ...
        let mut telemetry =
            TelemetryCollector::new(config.telemetry_mode, config.telemetry_capacity);
        ...
        let worker_result = catch_unwind(AssertUnwindSafe(|| {
            ...
            while !coordinator.is_finished() {
                ...
                local_metrics.playback_wall_time_us =
                    loop_start_us.saturating_sub(start_wall_time_us);
                ...
                if local_metrics.playback_wall_time_us > 0 {
                    local_metrics.spin_duty_cycle_ppm = (local_metrics.spin_time_us as u128
                        * 1_000_000
                ...
                if let Some((startup_scheduled_ticks, startup_lead_ticks)) = startup_gate {
                    ...
                    if target_sample_ticks < target_qpc {
                        ...
                        local_metrics.idle_wake_count =
                            local_metrics.idle_wake_count.saturating_add(1);
                ...
                let effective_now_ticks = if allow_pre_epoch_startup_dispatch
                ...
                let due_pending = match pending_plan.as_ref() {
                    Some(plan) => {
                        match coordinator.pop_due_pending_ticks(effective_now_ticks, plan) {
                            Ok(due) => due,
                            Err(error) => {
                                force_full_cleanup = true;
                                terminal_error =
                ...
                if !due_pending.is_empty() {
                    ...
                    let up_authored_completion_error_ticks = match signed_timeline_delta_ticks(
                        completed_effective_ticks,
                        scheduled_ticks,
                    ) {
                        Ok(value) => value,
                    ...
                    if recovery_required {
                        ...
                        let recovery_cleanup = backend
                            .release_all_full_instrument(target_hwnd.load(Ordering::Acquire));
                ...
                if let Some(prepared_batch) = prepared_batch {
                    ...
                    let authored_batch_scheduled_ticks =
                        coordinator.batch_scheduled_ticks[batch_index];
                        ...
                        if !scan_batch.is_empty() {
                            ...
                            match final_down_admission(
                                ...
                                DownAdmission::FocusLost => {
                                    ...
                                    if let Err(error) =
                                        clock_state.enter_pause("focus", focus_ticks)
                                    {
                                        force_full_cleanup = true;
                                        terminal_error = Some(format!(
                    ...
                    if deadline_ticks > target_sample_elapsed_ticks {
                            ...
                            .checked_add_duration(DurationTicks::from_raw(deadline_ticks.as_u64()))
                        {
                            ...
                            Err(error) => {
                                force_full_cleanup = true;
                                terminal_error =
                                    Some(format!("deadline arithmetic failure: {error}"));
                                break;
                            }
                        };
                            ...
                            Some(last_send_ticks) => {
                                    ...
                                    .checked_duration_since(last_send_ticks)
                                {
                                    ...
                                    Err(error) => {
                                        force_full_cleanup = true;
                                        terminal_error = Some(format!(
                                            "cold classification clock failure: {error}"
                                        ));
                                        break;
                                    }
                                };
                        ...
                        if config.input_path_warn_us > 0
                            && wake_error_us > config.input_path_warn_us
                        {
                            local_metrics.wait_path_degraded = true;
                        }
