...
pub use config::{DispatchProfile, TelemetryMode};
pub use snapshot::EngineSnapshot;
pub(crate) use telemetry::collector::*;
pub(crate) use telemetry::metrics::*;
pub(crate) use telemetry::trace::*;
pub use telemetry::trace::{
    NativeTelemetryOutput, NativeTelemetrySummary, RtTraceRecord, TimingSemantics,
};

#[cfg(any(test, feature = "test-support"))]
pub use test_support::command_timing::CommandTimingResult;

#[cfg(any(test, feature = "test-support"))]
pub(crate) use test_support::command_timing::*;

#[cfg(any(test, feature = "test-support"))]
pub(crate) use test_support::fault_injection::*;

#[cfg(any(test, feature = "test-support"))]
pub(crate) use test_support::mock_sender::*;
...
pub(crate) use worker::run_loop::*;
pub use worker::session::NativeDispatchSession;
pub(crate) use worker::session::*;

#[cfg(test)]
mod tests {
    use super::{
        CommandTimingError, CommandTimingResult, CommandTimingState, FaultInjectionScript,
        INPUT_PATH_WINDOW_CAPACITY, InjectedSendOutcome, NativeDispatchSession, PauseTimingLookup,
        PauseTimingPhase, PlatformSendResult, RtTraceRecord, SharedMetrics, TRACE_FLAG_SENT_FULL,
        TRACE_KIND_DOWN, TelemetryCollector, TelemetryMode, TraceContext, TraceDelivery,
        TraceTiming, TrackedKeyState, WakeErrorStats, WorkerMetricsLocal,
    };
    use crate::engine::telemetry::metrics::try_publish_metrics;
    use crate::engine::telemetry::trace::trace_outcome_code;
    use crate::engine::worker::run_loop::{
        DownAdmission, TargetStamp, adjust_spin_threshold, anchored_dispatch_target_ticks,
        ...
        supervisor_lease_expired, target_stamp_still_current, update_estimator_after_send,
        wake_lateness_ticks,
    };
    use sky_dispatch_core::estimator::{LatencyClass, SendLatencyEstimator};
