...
def check_calibration_cache() -> dict:
    """Checks whether the host sender hold-margin calibration cache exists."""
    ...
    if path.exists():
        ...
        status["ok"] = False
        status["msg"] = "Calibration cache not found. Run `--doctor-calibrate` to measure sender-side hold shrink for tighter hold margins."
    return status
...
def run_all_doctor_checks() -> bool:
    ...
    # 8. Host Sender Hold Calibration Cache (redundant path hint)
