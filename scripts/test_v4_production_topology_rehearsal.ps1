...
#
# Proves that:
#   1. The canonical verifiers (sbom verify, verify-tauri-bundle) accept a clean
#      2-file candidate-bundle + 6-file candidate-evidence fixture (happy path).
...
try {
    ...
    # ------------------------------------------------------------------
    # 1. Create candidate-bundle: exactly 2 files (installer + .sig)
    # ------------------------------------------------------------------
    $bundleDir = Join-Path $tmp "candidate-bundle"
    New-Item -ItemType Directory -Path $bundleDir -Force | Out-Null

    $installerName = "Sky Auto Player_4.1.1_x64-setup.exe"
    $sigName       = "$installerName.sig"
    $instPath      = Join-Path $bundleDir $installerName
    $sigPath       = Join-Path $bundleDir $sigName

    # Installer: 1 KiB of synthetic bytes (not a real PE, but sufficient for
    # verifier topology tests that only check structure, not code-signing).
    $installerBytes = [byte[]](1..100 | ForEach-Object { $_ % 256 }) * 10   # 1000 bytes
    ...
    # ------------------------------------------------------------------
    # 2. Create candidate-evidence: exactly 6 files
    # ------------------------------------------------------------------
    ...
    # Confirm candidate-evidence has exactly 6 files
    $evidenceFiles = @(Get-ChildItem -LiteralPath $evidenceDir -File)
    if ($evidenceFiles.Count -ne 6) {
        Fail "candidate-evidence must contain exactly 6 files; found $($evidenceFiles.Count)"
    }
    # Confirm candidate-bundle still has exactly 2 files
